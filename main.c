...
session_t *p_sess;//pointer point to an address
static unsigned int s_children;//son process number

static hash_t *s_ip_count_hash;//count and ip
static hash_t *s_pid_ip_hash;//pid and ip
...
int main(void)
{
	...
	daemon(0,0);
//printf some configure information once start system, because daemon process
...
//accept and corresponding update
//also fork() to have parent-child process
//then story begins
	while (1)
	{
		...
		if (pid == 0)
		{
			...
			begin_session(&sess);//here starts
		}
		else
		{
...
//check connects on all clients and on per ip
void check_limits(session_t *sess)
{
...
//when a client is disconnect,  the count should -1:
//1, find the ip of this client ----> hash: s_pid_ip_hash
//2, have known the ip, then s_ip_count_hash, count--

//1, through pid know the ip
void handle_sigchld(int sig)
{
...
//2, drop ip count, same as handle_ip_count() 
void drop_ip_count(void *ip)
{
