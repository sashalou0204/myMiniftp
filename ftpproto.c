...
int get_transfet_fd(session_t *sess);
int list_common(session_t *sess, int detail);//list current directory
...
void handle_alarm_timeout(int sig)//control connection
{
	shutdown(p_sess->ctrl_fd, SHUT_RD);// shut read only at first
	ftp_reply(p_sess, FTP_IDLE_TIMEOUT, "Timeout.");//cause we need to send a reply
	shutdown(p_sess->ctrl_fd, SHUT_WR);//then shut the write
	exit(EXIT_FAILURE);//exit
}

void handle_sigalrm(int sig)//data connection
{
	...
	start_data_alarm();//data,,but why recall back
}

void handle_sigurg(int sig)//when encouter the urge command?
{
	if (p_sess->data_fd == -1)
	{
		return;
	...
	char cmdline[MAX_COMMAND_LINE] = {0};//conctrol connection
	int ret = readline(p_sess->ctrl_fd, cmdline, MAX_COMMAND_LINE);//get the command
	if (ret <= 0)
	{
		ERR_EXIT("readline");
	}
	str_trim_crlf(cmdline);
	if (strcmp(cmdline, "ABOR") == 0//if its real abor
		|| strcmp(cmdline, "\377\364\377\362ABOR") == 0)
	{
		p_sess->abor_received = 1;//update the status
...
void check_abor(session_t *sess)
{
	...
	{
		sess->abor_received = 0;//update status again 
...
void start_cmdio_alarm(void)
{
	if (tunable_idle_session_timeout > 0)//control
	{
...
void start_data_alarm(void)
{
	...
	{
		signal(SIGALRM, handle_sigalrm);//signal do-recall function
		alarm(tunable_data_connection_timeout);//start alarm
	}
...
void handle_child(session_t *sess)
 {
     ...
     ftp_reply(sess, 220, "(miniftpd 0.1)");//say hello
     int ret;
	...
	start_cmdio_alarm();//check whether control connection is overtime

         ret = readline(sess->ctrl_fd, sess->cmdline, MAX_COMMAND_LINE);//receive command
         if(ret == -1)
                 ERR_EXIT("readline error");
         else if(ret == 0)//client has closed the connection
                 exit(EXIT_SUCCESS);//if client closed, ftp closed too
 
 
         //  "USER sasha/r/n" is waht we accepted, and we recognize USER as the command, the rest are all paras
 
     //  printf("cmdline = [%s]\n", sess->cmdline)//%s includes \r\n, so the output has two lines which is not we expectd
 
         //then remove the /r/n: employ those string-concreing function
     str_trim_crlf(sess->cmdline);
     printf("cmdline = [%s]\n", sess->cmdline);
 
         //prase FTP commands and para
     str_split(sess->cmdline, sess->cmd, sess->arg, ' ');//prase
     printf("cmd = [%s] arg = [%s]\n", sess->cmd, sess->arg);
     //process
     str_upper(sess->cmd);
...
{// opendir-->readdir-->stat
	...
	if(dir == NULL)
		return 0;
	struct dirent *dt;//traverse the directory and return a struct
	struct stat sbuf;
...
/*	mode_t mode = sbuf.st_mode;
	...
	char buf[1024] = {0};
*/
	const char *perms = statbuf_get_perms(&sbuf);//get the perms,all 10 
	int off = 0;
...
*/
	...
	const char *datebuf = statbuf_get_date(&sbuf);//get the date
	off += sprintf(buf+off, "%s", datebuf);
	
	if(S_ISLNK(sbuf.st_mode))//is linked?
	{
		char tmp[1024] = {0};
	...
	writen(sess->data_fd, buf, strlen(buf));//send to client	
}
	closedir(dir);
	return 1;
}
...
void limit_rate(session_t *sess, int bytes_transfered, int is_upload)
{
	...
	long curr_sec = get_time_sec();//get current time
	...
	// through bytes_transfered and elapsed time to get speed
	...
	if (is_upload)
	{
		...
		{
			// don't need to limit upload speed
			sess->bw_transfer_start_sec = curr_sec;//just update the start time
	...
	else
	{
		...
		{
			// don't need to limit the download speed
	...
	//sleeep time = (cur-speed / max-speed-1)* elapsed time
	// sleep time = (rate-ration -1) *elapsed time
	double pause_time;
	pause_time = (rate_ratio - (double)1) * elapsed;

	nano_sleep(pause_time);//sleep
...
//the port mode needs nobody process to help conecting
// using client address that FTP process has received
//we capsual this as a function

int get_port_fd(session_t *sess)
{
	/*
	send  nobody PRIV_SOCK_GET_DATA_SOCK       1
	port		       4
	and string : ip                       the length of ip is not sure
	*/

	priv_sock_send_cmd(sess->child_fd, PRIV_SOCK_GET_DATA_SOCK);
//to ask nobody process whether is in pasv mode
...
int get_pasv_fd(session_t *sess)//ask nobody
{
...
{
	//check whether the PORT or PASV
...
//if active mode
	if(port_active(sess))
	{
	//socket bind20 connect
	//self packege int tcp_client(unsigned short port)

//	tcp_client(20);
//	sess->data_fd = tcp_client(0);//supposed to tcp_client(20)
/*	int fd = tcp_client(0);
	if(connect_timeout(fd, sess->port_addr, tunable_connect_timeout) < 0)
	{
	//fail, then close the fd
		close(fd);
		return 0;
	}	
	sess->data_fd = fd;
...
/*		int fd = accept_timeout(sess->pasv_listen_fd, NULL, tunable_accept_timeout);
	...
	if(ret)
	{
		start_data_alarm();// restart SIGALARM and alarm
	}
	return ret;
}
 ...
 static void do_user(session_t *sess)
 {
     ...
     if(pw == NULL) //this user is not exit
     {
	ftp_reply(sess, FTP_LOGINERR, "1login_incorrect");
	return;
}
	// user exits, then ask to show the password
	sess->uid = pw->pw_uid;
 ...
 static void do_pass(session_t *sess)
 {
	...
	struct passwd *pw = getpwuid(sess->uid);// get struct passwd by uid
	if(pw == NULL)
	{
...
//char *crypt(const char *key, const char *salt);
	char *encrypted_pass = crypt(sess->arg, sp->sp_pwdp);//sp_pwdp is ciphertext from shadow
	//varify the password
	...
	setegid(pw->pw_gid);//set effective gid and uid,,but why?
	seteuid(pw->pw_uid);
	chdir(pw->pw_dir);//in user directory
...
static void do_quit(session_t *sess)
{
	ftp_reply(sess, FTP_GOODBYE, "Goodbye.");//reply first, then exit
	exit(EXIT_SUCCESS);
}
...
static void do_port(session_t *sess)
{
//PORT 192,168,0,100,123,233 parse the command
unsigned int v[6];
...
static void do_pasv(session_t *sess)
{
//supposed output: Entering Passive Mode (192,168,244,100,101,46)
	char ip[16] = {0};
	getlocalip(ip);
	
/*	sess->pasv_listen_fd = tcp_server(ip, 0);//want to know which port is binding
	struct sockaddr_in addr;//use getsockname func
	socklen_t addrlen = sizeof(addr);
	if(getsockname(sess->pasv_listen_fd, (struct sockaddr*)&addr,&addrlen) < 0)
	{
		ERR_EXIT("getsockname error");
	}//get the addr
	unsigned short port = ntohs(addr.sin_port);//store port

*/	
	priv_sock_send_cmd(sess->child_fd, PRIV_SOCK_PASV_LISTEN);
	unsigned short port = (int)priv_sock_get_int(sess->child_fd);

	unsigned int v[4];//store ip
	sscanf(ip, "%u.%u.%u.%u", &v[0], &v[1], &v[2], &v[3]);
	char text[1024] = {0};
	sprintf(text, "Entering Passive Mode (%u,%u,%u,%u,%u,%u).",v[0],v[1],v[2],v[3], port>>8, port&0xFF);//get high and low octets
...
static void do_retr(session_t *sess)//download and continuation
{
	if (get_transfer_fd(sess) == 0)//connection
	{
		return;
	}

	long long offset = sess->restart_pos;//for continuation
	sess->restart_pos = 0;
//download = open file + read context + wirte to fd
	...
	// only regular file can be downloaded
	struct stat sbuf;
	ret = fstat(fd, &sbuf);
	...
	if (offset != 0)
	{
		ret = lseek(fd, offset, SEEK_SET);//open from offset  
		if (ret == -1)
		{
			ftp_reply(sess, FTP_FILEFAIL, "Failed to open file.");
			return;
...
/* no limiting speed: read from fd then write to fd
char buf[4096];
while(1)
{
	...
	if(ret == -1)
	{
		if(errno == EINTER)//need to tell the reason from errno
		{
			continue;//intrupted
		}
		else
...
*/
	...
	//speed limitation: just use limit_rate(), and consider abor
	ftp_reply(sess, FTP_DATACONN, text);

	int flag = 0;
	long long bytes_to_send = sbuf.st_size;
	if (offset > bytes_to_send)//about expeerience
	{
		bytes_to_send = 0;
	}
	else
	...
	check_abor(sess);
	// reconnect and restart the alarm 
	start_cmdio_alarm();
	
}
void upload_common(session_t *sess, int is_append)
{
	if (get_transfer_fd(sess) == 0)//data connect
	{
		return;
	}
	...
	sess->bw_transfer_start_sec = get_time_sec();//for limiting speed
	sess->bw_transfer_start_usec = get_time_usec();
...
//create data connection
	...
	{return;}
//feedback a messege
	ftp_reply(sess,FTP_DATACONN,"Here conmes the directory listing.");
//start transferring list
	list_common(sess, 1);
//close data socket
	close(sess->data_fd);
	sess->data_fd = -1;
//give a postive feedback on the same time
