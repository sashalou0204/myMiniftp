...
//bind the address , ip = local ip; port = 20, only need to pass the port in
	if(port > 0)
	{
		int on = 1;
...
int tcp_server(const char* host, unsigned short port)
{
	...
	if((listenfd = socket(PF_INET, SOCK_STREAM, 0))<0)
			ERR_EXIT("tcp_server error");
	struct sockaddr_in servaddr;
	memset(&servaddr, 0, sizeof(servaddr));
	servaddr.sin_family = AF_INET;
	if(host != NULL)
	{
		//host is not null and host is not a valid ip address
		if(inet_aton(host, &servaddr.sin_addr) == 0)
		{
			//then host is the host-name
			struct hostent *hp;
	...
	else//host is nullptr,which means bind all the ip address
...
int connect_timeout(int fd, struct sockaddr_in *addr, unsigned int wait_seconds)
{
		...
		if(ret < 0 && errno == EINPROGRESS)
		{
				...
				{
						//when ret returns 1,it has two prosibilities: success 
						//or wrong in sockets which needs getsockopt to find the error
						int err;
...
ssize_t recv_peek(int sockfd, void *buf, size_t len)
{
		while(1)
		{
				int ret = recv(sockfd, buf, len, MSG_PEEK);//receive msg from socket
